...
class AnnoncesController extends Controller {
    ...
    private function handlePhotosUpload($id, $am) {
        ...
        for ($i = 0; $i < $n; $i++) {
            ...
            $url = $this->uploadToCloudinary($_FILES['photos']['tmp_name'][$i], $id);
            if ($url) $am->addPhoto($id, $url);
        }
    }

    private function uploadToCloudinary($tmpPath, $annonceId) {
        $cloudName = getenv('CLOUDINARY_CLOUD_NAME');
        $apiKey    = getenv('CLOUDINARY_API_KEY');
        $apiSecret = getenv('CLOUDINARY_API_SECRET');
        $timestamp = time();
        $publicId  = 'annonce_' . $annonceId . '_' . uniqid();
        $params    = ['public_id' => $publicId, 'timestamp' => $timestamp];
        ksort($params);
        $sigStr = urldecode(http_build_query($params)) . $apiSecret;
